...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { color?: keyof typeof brightColors }>(
  ({ className, color = "one", ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded-lg border border-gray-700 text-gray-200 shadow-sm",
        className
      )}
...
const CardFooter = React.forwardRef<HTMLDivElement, CardActionsProps>(
  ({ className, onDelete, onOpen, ...props }, ref) => (
    <div 
      ref={ref} 
      className={cn(
